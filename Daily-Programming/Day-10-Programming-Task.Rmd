...
The logistic function is defined as ${\cal L}(x) = \frac{e^x}{1 + e^x}$.    
Note that the function ${\cal L}(x)$ takes a scalar, $x$ as an input.

* Write a function `logistic()` that returns the value of the logistic function for any numerical input $x$.
...
* Write a function `LL_logistic()` that computes the log-likelihood of the observed result given a model (in the form of `coefs`).  It will have these arguments:
    ...
    * `outcome=` a `TRUE` or `FALSE` vector giving the outcome
...
```{r echo=params$show_answers, eval=params$show_answers}
source("Day-10-Test-Statements.R", local = TRUE)
```
