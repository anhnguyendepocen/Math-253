...
```
The `outer()` function takes two vectors and an operation. The operations is applied to every possible pair of an element from the first vector and an element from the second vector.
...
You will also find the `&` function useful, as in this test for a full house.^[There are two similar functions, `&` and `&&`. Use `&` when you want to compare vectors of any length.  You can use `&&` when combining vectors of length 1. `&&` (and its cousin `||`) doesn't evaluate additional conditions once it knows what the result will be.]
```{r eval=FALSE}
counts <- table(ranks(five_cards))
min(counts) == 1 & max(counts) == 2
```
...
```{r eval=show_answers, cache=TRUE}
table(sapply(1:100000, FUN=before_draw))
```

\newpage

```{r eval=show_answers, results="asis", echo=FALSE}
## Test statements
```

```{r tidy=FALSE}
source("Day-09-Test-Statements.R")
```
