...
Each individual trip's waiting time is modeled as random. You can choose whatever probability distribution you like, but it should be such that the probability density for negative values is zero (or, if you like, very small).  For instance, an exponential distribution might be appropriate.[^When using an exponential distribution, keep in mind that there is a hard cut-off at an input of zero: the probability of any negative value is exactly zero, meaning "impossible." (Zero likelihood corresponds to `-Inf` log-likelihood. Yet when searching for the maximum likelihood, you may encounter parameter values where `-Inf` is the result.  Sophisticated ethods of dealing with such boundaries are beyond the scope of this course, but you can always get around the problem by adding a small positive value to the likelihood before taking the log.)]  Use your fare model to make a maximum likelihood estimate of `base_fare`, `per_mile`, and the `params`. Plot `fare_amount` versus `trip_distance` as a scatter plot.  Then add a line showing the linear relationship you found, which you can do with `abline(base_fare, per_mile)`.
...
taxi_likelihood <- function(params, data=Taxi_trips) {
  ...
  #sum(log(dchisq(excess,rate))+1e-5)
...
ggplot(Taxi_trips, aes(x=trip_distance, y = fare_amount)) + 
  geom_point(alpha=.1) + 
