...
$$ \mbox{cov}(X_i, X_j) \defeq \mbox{E}\left[ (X_i - \mu_i)^T (X_j - \mu_i) \right]$$

The *covariance matrix* is ${\mathbf \Sigma}_{ij} \defeq  \mbox{cov}(X_i, X_j)$.  It is always a *symmetrical* matrix.
...
$${\mathbf A} \defeq \left[ \begin{array}{rr}
1.0 & -0.7\\
-0.7 & 1.0\\ 
\end{array} \right]$$

* Create a matrix object `A` to represent ${\mathbf A}$

```{r echo=show_answers}
A <- matrix(c(1, -0.7, -0.7, 1), nrow=2, ncol=2)
```

* Create a matrix object `A_inv` holding the inverse ${\mathbf A}^{-1}$.  (*Hint*: See `solve()`.)

```{r echo=show_answers}
A_inv <- solve(A)
```

You can construct the matrix product ${\mathbf A}{\mathbf A}^{-1}$ using the matrix multiplication operator `%*%` in R.

* Create a matrix object `test_inverse` that holds the product of `A` and `Ainv`.
...
* Create a matrix `A_chol` which is the Cholesky decomposition of A.
...
The *transpose* of a matrix is simply a flip around the diagonal.  In R, this is accomplished with the `t()` function. Calculate `t(A_chol)` and confirm that it is a *lower* triangular matrix.

Verify that the matrix product of `t(A_chol)` and `A_chol` is equivalent to A.
...
* Create matrix `A_inv_chol` which is the Cholesky decomposition of the inverse of A. (Note that `A_inv_chol` is meant to imply "Take the inverse of A, and then the Cholesky decomposition of that.")
```{r echo=show_answers}
A_inv_chol <- chol(solve(A))
```

* Multiply `X` by `A_chol`.  The result --- call it `Y` --- should be a matrix of the same dimensions as `X`.

```{r echo=show_answers}
Y <- X %*% A_chol
```

The covariance matrix of `Y` should be similar to `A`, but `Y` is so short that there will be lots of random fluctuations.

```{r echo=show_answers, eval=show_answers}
t(Y) %*% Y / nrow(Y)
```

* Resassign `Y` to be the product of `W` times `A_chol`.  This will give you 10,000 random samples with a covariance matrix close to `A`.  

Check whether the covariance matrix of this new, bigger `Y` is close to `A`.  The difference between `A` and the covariance matrix of `Y` should be similar in size to $1/\sqrt{n}$.  Is it?
...
```{r echo=show_answers, eval=show_answers}
source("Day-11-Test-Statements.R")
```
